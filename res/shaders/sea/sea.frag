...
vec3 calculateLight(DirLight light, vec3 normal, vec3 fragPos, vec3 viewDir) {
    ...
    return (specular) * light.strength;
}
...
//    float cosNV = dot(normal, viewDir);
    ...
    vec3 scatteringColor = watercolor;// vec3(0,1.0,0.95);
    if(underwater) {
...
void main() {
    ...
    if(fragPos3D.x < 0.5 && fragPos3D.x > -0.5 && fragPos3D.z < 0.5 && fragPos3D.z > -0.5) discard;
    gl_FragDepth = ((gl_DepthRange.diff * computeDepth(fragPos3D)) + gl_DepthRange.near + gl_DepthRange.far) / 2.0;
    vec3 fragPos = fragPos3D;

    vec2 texCoords = vec2(fragPos.z, fragPos.x);

    vec4 albedo = material.albedo;
    vec3 viewDir = normalize(viewPos - fragPos);
    vec3 normNormal = vec3(0,1,0);

    normNormal = normalize(normNormal + perlinNormal(texCoords));
//    normNormal = normalize(normNormal + windWaves(texCoords));

    vec3 result = vec3(0);//vec3(0.2f) *  * dirLight.color;
    result += waterColor(normNormal, viewDir, fragPos, albedo.rgb);// * 0.4;
...
//    fragColor = vec4(normNormal, 1);
    fragColor = vec4(result, /*albedo.a * fading*/1);
}
